...
describe.only('Checkbox', () => {
	...
	it('should set the status to indeterminate when indeterminate flag is true', () => {
		const component = mount(<Checkbox checked={false} indeterminate />);
	...
	it('should not do anything when there is not onPress handlers', async () => {
		const BBCheckbox = getComponent('Checkbox');

		const component = mount(
			<BlueBaseApp components={{ Checkbox }}>
				<BBCheckbox color="default" value={1} />
			</BlueBaseApp>
		);
